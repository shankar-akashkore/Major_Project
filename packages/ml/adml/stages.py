...
class Harvest:
    ...
    @property
    def n_premium_sets(self) -> int:
        """Sets generated by the paid tier.

        The report used to infer this from total spend being above zero, which
        cannot tell a $0.39 pair of contract smoke tests from a $2.22 job that
        actually ran the whole pipeline — and so kept printing "no full job has
        been generated" after one had. The tier is recorded per candidate; read it
        instead of guessing from a number that means something else.
        """
        return sum(1 for o in self.observations if o.tier == "premium")
